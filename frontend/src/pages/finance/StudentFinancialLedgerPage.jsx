...
export default function StudentFinancialLedgerPage() {
  ...
  const studentName = ledger?.student?.name || ledger?.student_name || 'Student';
  const admissionNo = ledger?.student?.admission_no || ledger?.admission_no || '—';
  const className = ledger?.student?.class_name || ledger?.class_name || '—';
  const fatherName = ledger?.student?.father_name || ledger?.father_name || '—';
  const parentPhone = ledger?.student?.parent_phone || ledger?.parent_phone || '—';
  const rollNo = ledger?.student?.roll_no || ledger?.roll_no || '—';

  const bills = ledger?.bills || [];
  const payments = ledger?.payments || [];
  const ledgerEntries = ledger?.ledger_entries || ledger?.ledger_movements || [];
  const concessions = ledger?.concessions || [];
          ...
          <div className="page-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12 }}>
            <div>
              ...
              <h2 className="page-title">
                {studentName} — Financial Ledger
              </h2>
              <p className="page-subtitle">
                Complete debit/credit ledger, advance demand notices, payment receipts, and scholarships.
              </p>
            </div>
            ...
            <>
              ...
              <div className="grid-4 mb-6">
                <div className="stat-card" style={{ borderLeft: '4px solid #0176d3' }}>
                  <div className="stat-label">Student Information</div>
                  <div style={{ fontSize: '1.125rem', fontWeight: 800, color: 'var(--neutral-9)' }}>
                    {studentName}
                  </div>
                  <div className="stat-sub">
                    {admissionNo} • {className} {rollNo !== '—' ? `(Roll: ${rollNo})` : ''}
                  </div>
                  <div className="text-xs text-muted" style={{ marginTop: 4 }}>
                    Father: <strong>{fatherName}</strong> ({parentPhone})
                  </div>
                </div>
                ...
                <div className="stat-card" style={{ borderLeft: '4px solid #0176d3' }}>
                  ...
                  <div className="stat-sub">{bills.length} demand bills issued</div>
                </div>
                ...
                <div className="stat-card" style={{ borderLeft: '4px solid #2e844a' }}>
                  ...
                  <div className="stat-sub">{payments.length} payment receipts</div>
                </div>

                <div className="stat-card" style={{ borderLeft: `4px solid ${(ledger.outstanding || 0) > 0 ? '#dd7a01' : '#2e844a'}` }}>
                  <div className="stat-label">Pending Dues / Balance</div>
                  <div className="stat-value" style={{ fontSize: '1.5rem', color: (ledger.outstanding || 0) > 0 ? '#dd7a01' : '#2e844a' }}>
                    {fmt(ledger.outstanding)}
                  </div>
                  <div className="stat-sub">
                    {(ledger.advance_credit || ledger.advance_balance || 0) > 0
                      ? `Advance Credit: ${fmt(ledger.advance_credit || ledger.advance_balance)}`
                      : 'Current Pending Arrears'}
                  </div>
                </div>
              </div>
              ...
              <div className="card mb-6">
                <div className="card-header" style={{ padding: '8px 16px', background: '#fafaf9' }}>
                  <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                    {[
                      { id: 'bills', label: 'Demand Bills', icon: 'ti-file-invoice', count: bills.length },
                      { id: 'receipts', label: 'Payment Receipts', icon: 'ti-receipt', count: payments.length },
                      { id: 'movements', label: 'Ledger Audit Trail', icon: 'ti-list-check', count: ledgerEntries.length },
                  ...
                  <div className="table-container" style={{ border: 'none' }}>
                    {bills.length === 0 ? (
                      <div className="empty-state">
                        ...
                        <h4 style={{ marginTop: 8 }}>No Fee Bills Issued Yet</h4>
                      </div>
                    ) : (
                        ...
                        <tbody>
                          {bills.map((b) => (
                            <tr key={b.id}>
                              ...
                              <td style={{ fontWeight: 600 }}>{b.bill_period_label || b.bill_month}</td>
                              <td>{b.generation_date || b.issue_date}</td>
                  ...
                  <div className="table-container" style={{ border: 'none' }}>
                    {payments.length === 0 ? (
                      <div className="empty-state">
                        ...
                        <h4 style={{ marginTop: 8 }}>No Payments on Record</h4>
                      </div>
                    ) : (
                      <table className="table">
                        <thead>
                ...
                {activeTab === 'movements' && (
                  <div className="table-container" style={{ border: 'none' }}>
                    {ledgerEntries.length === 0 ? (
                      <div className="empty-state">
                        <i className="ti ti-list-check" style={{ fontSize: 36, color: 'var(--neutral-4)' }}></i>
                        <h4 style={{ marginTop: 8 }}>No Ledger Entries</h4>
                      </div>
                    ) : (
                      <table className="table">
                        <thead>
                        ...
                        <tbody>
                          {ledgerEntries.map((m) => (
                            <tr key={m.id}>
                              <td style={{ fontSize: 12, fontWeight: 500 }}>{m.entry_date || m.created_at?.slice(0, 10)}</td>
                              <td>
                                <span className={`badge ${m.entry_type === 'CREDIT' ? 'badge-success' : 'badge-warning'}`}>
                                  {m.entry_type}
                                </span>
                              </td>
                              <td style={{ fontWeight: 600 }}>{m.fee_head_name}</td>
                              <td style={{ fontSize: 12, color: 'var(--neutral-6)' }}>{m.description}</td>
                              <td style={{ textAlign: 'right', fontWeight: 700, color: m.entry_type === 'DEBIT' ? '#ba0517' : '#2e844a' }}>
                                {m.entry_type === 'DEBIT' ? `+ ${fmt(m.amount)}` : `- ${fmt(m.amount)}`}
                              </td>
