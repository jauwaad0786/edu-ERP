...
function StatusPill({ status }) {
  ...
  return <span style={{ background: s.bg, color: s.color, padding: '2px 9px', borderRadius: 100, fontSize: 10, fontWeight: 700 }}>{status}</span>;
}
...
export default function FeeStructures() {
  ...
  // NEW — component body ke andar, existing states ke baad, adjustment-tab states add karo
  ...
  async function submitAdjustment() {
    if (!adjAmount || isNaN(adjAmount) || Number(adjAmount) <= 0) {
      toast.error('Sahi amount daalo'); return;
    }
    if (!adjReason.trim()) {
      toast.error('Reason zaroori hai'); return;
    }
    setAdjSaving(true);
    try {
    ...
    <div className="main-content">
      ...
      <div className="page-body">
        <div className="page-header">
          <div>
            ...
            <p className="page-subtitle">
              {activeTab === 'rates' ? 'Class-wise rate card — Generate Fees isi se amount uthata hai' : 'Fine lagao ya fees maaf karo — kisi bhi student ke kisi bhi record pe'}
            </p>
          </div>
          {activeTab === 'rates' && (
        ...
        <div style={{ display: 'flex', gap: 4, marginBottom: 20, borderBottom: '1px solid #e2e8f0' }}>
          {[['rates', '📋 Rate Card'], ['adjustments', '⚖️ Adjustments']].map(([key, label]) => (
            <button key={key} onClick={() => setActiveTab(key)}
              style={{
                padding: '10px 18px', fontSize: 13, fontWeight: 700, cursor: 'pointer',
                border: 'none', background: 'none',
                color: activeTab === key ? '#0176d3' : '#64748b',
                borderBottom: activeTab === key ? '2px solid #0176d3' : '2px solid transparent',
              }}>{label}</button>
          ))}
        </div>
        ...
        <select className="form-select" style={{ width: 220, marginBottom: 16 }}
          ...
          {loading ? (
            ...
            // NEW — close the rates fragment, then insert Adjustments tab
            </div>
          )}
        </>)}
          ...
          <div className="modal" style={{ maxWidth: 400 }}>
            ...
            <div className="modal-body">
              ...
              <input type="number" value={adjAmount} onChange={e => setAdjAmount(e.target.value)}
                style={{ width: '100%', padding: '9px 12px', fontSize: 13, border: '1px solid #e2e8f0', borderRadius: 8, marginBottom: 10, boxSizing: 'border-box' }} />
              <label style={{ fontSize: 11, fontWeight: 600, color: '#64748b', display: 'block', marginBottom: 4 }}>Reason *</label>
              <input value={adjReason} onChange={e => setAdjReason(e.target.value)}
                ...
                style={{ width: '100%', padding: '9px 12px', fontSize: 13, border: '1px solid #e2e8f0', borderRadius: 8, boxSizing: 'border-box' }} />
            </div>
            <div className="modal-footer">
              <button className="btn btn-neutral" onClick={() => setAdjustModal(null)}>Cancel</button>
              <button onClick={submitAdjustment} disabled={adjSaving}
                style={{ background: adjustModal.type === 'FINE' ? '#ba0517' : '#2e844a', color: '#fff', border: 'none', borderRadius: 6, padding: '8px 20px', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>
