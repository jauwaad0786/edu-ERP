...
export default function StudentsScreen() {
  ...
  const load = useCallback(async (isRefresh = false) => {
    ...
    try {
      const res = await client.get('/principal/students').catch(() => ({ data: [] }));
      ...
      >
        {filtered.length > 0 ? (
            ...
            <View key={s.id || i} style={styles.card}>
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12 }}>
                <View style={styles.avatar}>
                  <Text style={styles.avatarText}>{(s.name || 'S').charAt(0).toUpperCase()}</Text>
                </View>
                <View style={{ flex: 1 }}>
                  <Text style={styles.name}>{s.name}</Text>
                  <Text style={styles.sub}>
                    Class {s.class_name || s.class_obj?.name || '—'} {s.section ? `(${s.section})` : ''} · Roll: {s.roll_no || '—'}
                  </Text>
                  <Text style={{ fontSize: 11, color: C.muted, marginTop: 2 }}>Adm No: {s.admission_no || '—'}</Text>
                </View>
                <View style={[styles.statusBadge, { backgroundColor: s.status === 'INACTIVE' ? '#fee2e2' : '#dcfce7' }]}>
                  <Text style={{ fontSize: 10, fontWeight: '700', color: s.status === 'INACTIVE' ? C.error : C.green }}>
                    {s.status || 'ACTIVE'}
                  </Text>
                </View>
              </View>
          ...
          <View style={{ alignItems: 'center', padding: 40 }}>
            ...
            <Text style={{ color: C.muted, marginTop: 12, fontSize: 14 }}>No students found</Text>
          </View>
        )}
      </ScrollView>
    </SafeAreaView>
